...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
        ...
        imgsz=640,  # inference size (pixels) 这块儿有改动
    ...
    # 删除了is_file和check_file,但是is_url直接写在下面了
    ...
    # Initialize 这个应该是后加的，保证用gpu
    set_logging()
    device = select_device(device)
    half &= device.type != 'cpu'  # half precision only supported on CUDA

    # Load model 改动较多,直接把DetectMultiBackend粘贴过来了，但是有改动，类似于对is_url的处理
    w = str(weights[0] if isinstance(weights, list) else weights)
    classify, suffix, suffixes = False, Path(w).suffix.lower(), ['.pt', '.onnx', '.tflite', '.pb', '']
    check_suffix(w, suffixes)  # check weights have acceptable suffix
    pt, onnx, tflite, pb, saved_model = (suffix == x for x in suffixes)  # backend booleans
    stride, names = 64, [f'class{i}' for i in range(1000)]  # assign defaults 步长改为64
    if pt:
        ...
        if half:
            model.half()  # to FP16
        if classify:  # second-stage classifier 这个是自己加的
    ...
    imgsz = check_img_size(imgsz, s=stride)  # check image size # imgsz是640
            ...
            if len(det):
                ...
                for d in det:
                    ...
                    if save_crop:
                        ...
                        file = save_dir / 'crops' / names[int(c)] / p.name
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/train/8-1-test/weights/best.pt',
                        help='model path(s)')
    parser.add_argument('--source', type=str, default='data/calib', help='file/dir/URL/glob, 0 for webcam')
    ...
    print_args(FILE.stem, opt) # 可能改了，不影响实际作用
    return opt
